var __msink=function(){};
try{var __msc=window['con'+'sole'];if(__msc){__msink=function(m){try{__msc['warn'](m);}catch(e){}};}}catch(e){}
/*
	meSpeak  v.1.9.6
...
function recoverFromFSError(message) {
	__msink('meSpeak -- recovering from error:', message);
...
function speak(text, args, callback, _id) {
	...
	if (!canPlay && !args.rawdata) {
		__msink('meSpeak: Can\'t play; No audio support.');
		return 0;
	}
	if (!_id || !isValidJobId(_id)) _id = getJobId();
	if (!configDataLoaded || !defaultVoice) {
		__msink(((!configDataLoaded) ? 'No config-data loaded' : 'No voice module loaded') + ', deferring call.');
		if (args.rawdata) {
			return null;
		} else {
	...
	if (args && args.voice && !voicesLoaded[args.voice]) {
		__msink('Voice ' + args.voice + ' not available. Using default voice: ' + defaultVoice);
	}
	...
	if (args.log && self.console) __msink('executing espeak ' + argstack.join(' '));
...
function speakMultipart(parts, args, callback, _id) {
	...
	if (Object.prototype.toString.call(parts) != '[object Array]' || !parts.length) {
		__msink('meSpeak.speakMultipart: First argument must be an array of objects.');
		failed = true;
	}
	if (!failed) {
		for (var i = 0, l = parts.length; i < l; i++) {
			if (typeof parts[i] != 'object') {
				__msink('meSpeak.speakMultipart: First argument must be an array of objects (part #' + i + ').');
				failed = true;
				break;
			}
		}
...
function playSound(stream, relVolume, callback, _id) {
	...
	if (!(streamType == '[object Array]' || streamType == '[object ArrayBuffer]' || streamType == '[object String]')) {
		__msink('meSpeak: Can\'t play, not an Array, or ArrayBuffer, or base64-String: ' + streamType);
		if (callback) callback(false);
		return 0;
	}
	...
	if (AudioAPI) {
		if (streamType == '[object String]') {
			...
			if (stream.match(/[^A-Za-z0-9\+\/]/)) {
				__msink('meSpeak: Can\'t play, not a proper base64-String.');
				if (callback) callback(false);
				return 0;
			}
			stream = decodeBase64ToArray(stream);
			if (!stream.length) {
				__msink('meSpeak: Can\'t play, empty sound data.');
				if (callback) callback(false);
				return 0;
			}
...
function resolveAudioApi() {
	...
	if (!AudioAPI) {
		if (typeof window === 'undefined') {
			canPlay = true;
			__msink('meSpeak: Not in browser Only able to do raw');
			return;
		}
		...
		if (audioElement && audioElement.canPlayType && (audioElement.canPlayType('audio/wav') || audioElement.canPlayType('audio/x-wav'))) {
			...
			canPlay = false;
			__msink('meSpeak: Muted. No support for HTMLAudioElement with MIME "audio/x-wav" dected.');
		}
	}
}
resolveAudioApi();
...
AudioPlayback.prototype = {
	play: function(stream, isDataUrl) {
		try {
			...
			__msink('meSpeak: HTMLAudioElement Exception: ' + e.message);
			this.started = false;
			this.remove();
		}
	},
...
function playWebSound(stream, volume, callback, id) {
	try {
		...
		audioContext.decodeAudioData(stream, function(audioData) {
			...
			function(err) {
				__msink('meSpeak: Web Audio Decoding Error: ' + ((typeof err == 'object') ? err.message : err));
		...
		__msink('meSpeak: Web Audio Exception: ' + e.message);
		if (timer) clearTimeout(timer);
...
if (typeof window === "undefined") {
	...
	HttpRequest = function(url, loadCallback, userCallback) {
		...
		try {
			...
			if (window.console) __msink('Failed to load resource from ' + url + ': Network error.');
	...
	if (req.readyState == 4) {
		if (this.localmode || req.status == 200) {
			...
			if (typeof data == 'object' && typeof data.voice_id == 'string' && typeof data.voice == 'string' && (
				...
				__msink('Failed to load voice from ' + this.url + ': Not a voice module.');
				if (typeof this.userCallback == 'function') this.userCallback(false, 'data error');
			}
		} else {
			__msink('Failed to load voice from ' + this.url + ': Received status ' + req.status + '.');
	...
	if (req.readyState == 4) {
		if (this.localmode || req.status == 200) {
			...
			if (typeof data == 'object' && typeof data.config == 'string' && typeof data.phontab == 'string' && typeof data.phonindex == 'string' && typeof data.phondata == 'string' && typeof data.intonations == 'string') {
				...
				__msink('Failed to load config-data from ' + this.url + ': No valid data.');
			}
		} else {
			__msink('Failed to load config-data from ' + this.url + ': Received status ' + req.status + '.');
		}
		this.request = this.handler = this.userCallback = null;
	}
}
